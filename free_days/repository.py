...
def get_user_projects(user_workspace):
    projects = requests.get(f"https://api.clockify.me/api/v1/workspaces/{user_workspace}/projects",
                            headers={"X-Api-Key": "MDU3YmYxZDYtYjVhZC00N2VjLThmOGUtNGUxNGMyNmEzZmQ2"})
...
def get_time_records(project_id, user_id, workspace):
    ...
    time_records = requests.get(f"https://api.clockify.me/api/v1/workspaces/{workspace}/user/{user_id}/time-entries?start={year}-01-01T00:00:00.00Z&end={year}-12-31T00:00:00.00Z&project={project_id}",
                                headers={"X-Api-Key": "MDU3YmYxZDYtYjVhZC00N2VjLThmOGUtNGUxNGMyNmEzZmQ2"})
    return json.loads(time_records.text)


def get_paid_time_off_task_id(workspace, project_id):
    tasks = requests.get(f"https://api.clockify.me/api/v1/workspaces/{workspace}/projects/{project_id}/tasks",
                         headers={"X-Api-Key": "MDU3YmYxZDYtYjVhZC00N2VjLThmOGUtNGUxNGMyNmEzZmQ2"})
